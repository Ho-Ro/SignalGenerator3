...
const uint8_t digits = 6; // number of digits ( nOD ) in the frequency display
...
const uint8_t WvFormPos  = 2 * digits;
...
//-----------------------------------------------------------------------------
unsigned long calcFreq( uint8_t* freqSG ) {
  unsigned long f = 0;
...
void decItem( void ) {
  ...
  } else if ( cursor == SweepPos ) { // 1, 3, 10, 30
    if ( sweep == swOff )
      sweep = sw30Sec;
    else
      sweep = sweep_t( sweep - 1 );
